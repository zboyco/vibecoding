---
name: workflow
description: 仅在用户显式指定调用时激活的六阶段工作流；支持研究阶段确认后进入快速完成。
---

# 显式调用时生效的六阶段工作流技能（支持研究后快速完成）

## 职责边界

- 本技能仅在用户显式指定调用时激活，例如明确要求使用 `$workflow` 或按本技能的六阶段流程执行。
- 未被用户显式指定时，不得因“新需求”“任务复杂”“历史习惯”或其他主观判断自动启用本技能。
- 本技能一旦激活，即成为当前任务的流程入口。
...
- 本技能只负责阶段流程控制，不替代工具选择规则。
- 涉及 Go 代码理解、诊断、符号查询、引用分析、包 API 确认、重命名或漏洞检查时，必须遵守当前已加载的全局代理指令文件中的 `gopls` MCP 工具使用提示。
- 当目标文件、包、类型、函数或方法尚不明确时，仍先使用 `mcp-search-context` 定位业务上下文；明确到 Go 文件、包或符号后，再按当前已加载的全局代理指令文件中的 `gopls` 使用顺序选择具体工具。
- 在执行模式或快速完成模式中修改 Go 代码后，应使用 `go_diagnostics` 做基础诊断；这不替代必要的测试。

## 全局强制约束

- 始终使用中文交流和回复。
- 查找或搜索代码时，优先使用 `mcp search_context`。
- 本技能激活后，默认按固定顺序执行：`研究 -> 构思 -> 计划 -> 执行 -> 优化 -> 评审`。
...
> 如果使用 mermaid，应只表达“流程/状态/协作关系”这类结构信息，不要把整段总结原样搬进图里。
> mermaid 适合补充说明，不替代“已完成 / 关键结论 / 风险与阻塞”的文字总结。
...
- 应根据内容复杂度自行判断使用列表、表格、mermaid或三者结合；不强制每次同时使用三者。
- 当信息是状态汇总、字段对比、结果对照时，优先考虑表格；当信息是建议、行动项、反馈选项或线性说明时，优先考虑列表；当信息是结构关系、处理路径、调用顺序、业务状态变化时，优先考虑 mermaid 图示。
- mermaid 的主要用途是“让结构一眼可见”，不是“压缩文字篇幅”；如果 3 到 5 条列表已经足够清楚，则不要强行画图。
- mermaid 图应只覆盖一个核心主题，例如“模块关系”“数据流向”“一次请求的处理路径”“服务间交互顺序”或“业务状态变化”；不要在一张图里混入过多目标。
- 图中节点文案应简短，优先使用短语而非整句或段落；详细说明仍放在图外的列表或表格中。
- 推荐图种选择：
  - `flowchart`：适合表达处理流程、依赖路径、分支判断。
  - `flowchart` + `subgraph`：适合表达按模块、服务或责任边界划分的“泳道式”结构。
  - `sequenceDiagram`：适合表达模块、服务、系统之间的交互顺序。
  - `stateDiagram-v2`：适合表达业务对象的状态变化与转换条件。
- 若使用 mermaid，优先放在“## 📌 本阶段总结”下方、`## 🚦 建议下一步`之前，并紧邻对应的总结文本，避免图文分散。
- mermaid 示例（用于展示一段通用处理链路，可按实际总结内容改写）：
...
1. 在用户已显式指定调用本技能的前提下，识别当前所处阶段；若是新需求，固定进入 `[模式：研究]`。
